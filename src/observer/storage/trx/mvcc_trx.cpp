...
RC MvccTrxKit::init()
{
  fields_ = vector<FieldMeta>{
    FieldMeta("__trx_xid_begin", AttrType::INTS, 0/*attr_offset*/, 4/*attr_len*/,true /*nullable*/, false/*visible*/),
    FieldMeta("__trx_xid_end",   AttrType::INTS, 0/*attr_offset*/, 4/*attr_len*/,true /*nullable*/, false/*visible*/)
  };
...
RC MvccTrx::insert_record(Table *table, Record &record, bool mvcc_unique_update)
{
  Field begin_field;
  Field end_field;
  trx_fields(table, begin_field, end_field);
  ...
  RC rc = table->insert_record(record, mvcc_unique_update);
  if (rc != RC::SUCCESS) {
    LOG_WARN("failed to insert record into table. rc=%s", strrc(rc));
    return rc;
  }
...
RC MvccTrx::update_record(Table *table, Record &old_record, Record &new_record)
{
    ...
    rc = insert_record(table, new_record, true);
    if (rc != RC::SUCCESS) {

...
RC DefaultConditionFilter::init(Table &table, const ConditionSqlNode &condition)
{
  ...
  if (CON_ATTR_T == condition.left_type) {
    left.is_attr = true;
  ...
  } else if (CON_VALUE_T == condition.left_type){
    left.is_attr = false;
  ...
  if (CON_ATTR_T == condition.right_type) {
    right.is_attr = true;
  ...
  } else if (CON_VALUE_T == condition.right_type) {
    right.is_attr = false;
...
bool DefaultConditionFilter::filter(const Record &rec) const
{
  ...
  return left_value.compare(right_value,comp_op_);
}

...
RC FilterStmt::create(Db *db, Table *default_table,
  ...
  for (int i = 0; i < condition_num; i++) {
    ...
    rc = create_filter_unit_expr(db, default_table, tables, conditions[i],
                                 filter_unit);
    if (rc != RC::SUCCESS) {
      delete tmp_stmt;
    ...
    const ConditionSqlNode &condition, FilterUnit *&filter_unit)
{
  ...
  // 左expression
  switch (condition.left_con_type) {
  case CON_ATTR_T: {
    Table *table = nullptr;
    const FieldMeta *field = nullptr;
    rc = get_table_and_field(db, default_table, tables,
                             condition.left_expr_node.attributes[0], table,
                             field);
    if (rc != RC::SUCCESS) {
      LOG_WARN("cannot find attr");
      return rc;
  ...
  case CON_VALUE_T: {
    ...
    ValueExpr *value_expr =
        static_cast<ValueExpr *>(condition.left_expr_node.expression);
  ...
  // 右expression
  switch (condition.right_con_type) {
  case CON_ATTR_T: {
    Table *table = nullptr;
    const FieldMeta *field = nullptr;
    rc = get_table_and_field(db, default_table, tables,
                             condition.right_expr_node.attributes[0], table,
                             field);
    if (rc != RC::SUCCESS) {
      LOG_WARN("cannot find attr");
      return rc;
  ...
  case CON_VALUE_T: {
    FilterObj filter_obj;
    ValueExpr *value_expr =
        static_cast<ValueExpr *>(condition.right_expr_node.expression);
    filter_obj.init_value(value_expr->get_value());
    filter_unit->set_right(filter_obj);
  } break;
  case CON_SUBSELECT_T:
